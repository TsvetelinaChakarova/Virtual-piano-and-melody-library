...
class VirtualPianoFrame:
    ...
    def create_fields(self, login_frame, view_melodies_frame, view_global_melodies_frame):
        record_melody_by_keys_button = tk.Button(self.virtual_piano_frame,
                                                 text='Record a melody by pressing piano keys',
                                                 command=lambda: [record_melody_by_pressing_keys.record_melody(),
                                                                  self.show_message_for_recording()])
        record_melody_by_keys_button.pack()

        stop_record_melody_button = tk.Button(self.virtual_piano_frame,
                                              text='Stop recording the melody by pressing piano keys',
                                              command=lambda: [record_melody_by_pressing_keys.stop_recording_melody(
                                                  self.virtual_piano_frame), self.stop_showing_message_for_recording()])
        stop_record_melody_button.pack()

        record_melody_by_text_button = tk.Button(self.virtual_piano_frame,
                                                 text='Record a melody by inserting notes as text', command=lambda: [
                record_melody_by_text.save_melody_popup_window(self.virtual_piano_frame)])
        ...
        view_global_melodies_button = tk.Button(self.virtual_piano_frame, text="View global melodies",
                                                command=lambda: [
                                                    self.change_to_view_melodies_frame(view_global_melodies_frame)])
