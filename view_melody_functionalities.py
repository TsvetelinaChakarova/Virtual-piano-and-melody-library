...
class ViewMelodiesFrame:
    ...
    def play_melody(self):
        ...
        path_to_melody = "Melodys\\" + login_frame_functionalities.current_user_username + "\\" + self.treeview.item(selected_item)['values'][0] + ".wav"
        pygame.mixer.init()
    ...
    def search_melody(self, search_input_field):
        ...
        cursor = cursor.execute("SELECT name FROM melodys WHERE creators_username = '" + login_frame_functionalities.current_user_username + "' AND (keywords LIKE '" + keyword + " %' or keywords LIKE '% " + keyword + "' or keywords LIKE '% " + keyword + " %' or keywords LIKE '" + keyword + "')")
        rows = cursor.fetchall()    
        for row in rows:
            self.treeview.insert("", tk.END, values=row)  
